...
namespace cvedix_utils {
    ...
    private:
        ...
        *                                                                                           
        */
        void check_layer(std::vector<std::shared_ptr<cvedix_nodes::cvedix_node>> nodes_in_layer) {
            num_layers_in_pipe++;

            int next_nodes_num = -1;
            std::vector<std::shared_ptr<cvedix_nodes::cvedix_node>> all_next_nodes;
            ...
            for(auto& i : nodes_in_layer) {
                // check for the first layer, all nodes MUST be cvedix_nodes::cvedix_node_type::SRC
                if (num_layers_in_pipe == 1 && 
                    i->node_type() != cvedix_nodes::cvedix_node_type::SRC) {
                    throw cvedix_excepts::cvedix_invalid_pipeline_error("pipe MUST starts with SRC nodes!");
                }
                ...
                if (std::find_if(std::begin(node_names_in_pipe), 
                                ...
                                != 
                                std::end(node_names_in_pipe)) {
                    throw cvedix_excepts::cvedix_invalid_pipeline_error("node names MUST not be duplicated in pipe!"); 
                }              
                ...
                // check for the last layer, all nodes MUST be cvedix_nodes::cvedix_node_type::DES
                if (_num == 0 && i->node_type() != cvedix_nodes::cvedix_node_type::DES) {
                    throw cvedix_excepts::cvedix_invalid_pipeline_error("pipe MUST ends with DES nodes!");
                }
                ...
                if (next_nodes_num != _num) {
                    throw cvedix_excepts::cvedix_invalid_pipeline_error("all nodes in the same layer MUST have the same next nodes number!"); 
                }
            ...
            if (all_next_nodes.size() != 0) {
                ...
                if (!all_the_same && !all_distinct) {
                    throw cvedix_excepts::cvedix_invalid_pipeline_error("the next nodes of each node in the same layer MUST be the same or be distinct totally!"); 
                }
        ...
        bool summary;
    public:
        cvedix_pipe_checker(bool summary = true) {
            summary = summary;
        }
        ...
        void operator()(std::vector<std::shared_ptr<cvedix_nodes::cvedix_node>> pipe_src_nodes) {
            ...
            if (summary) {
                ...
                */
                ...
                v_stream << "\n################ version info #################\n";
                v_stream << " build_time: " << CVEDIX_BUILD_TIME << "\n";
                v_stream << " commit_hash: " << CVEDIX_GIT_COMMIT << "\n";
                v_stream << " version: " << CVEDIX_VERSION << "\n";
                v_stream << "################ version info ################" << "\n";
                // to log
                CVEDIX_INFO(v_stream.str());
