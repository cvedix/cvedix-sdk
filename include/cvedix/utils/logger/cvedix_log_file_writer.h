...
namespace cvedix_utils {
    ...
    private:
        ...
        std::string log_dir;
        // log file name template, such as `<year>-<mon>-<day>.txt`, refer to cvedix_utils::time_format(...) for rule details
    ...
    public:
        ...
        // for << operator
        cvedix_log_file_writer& operator<<(std::string log);
    };
}