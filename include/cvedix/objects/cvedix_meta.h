...
namespace cvedix_objects {
    ...
    protected:
        ...
        */
        //std::map<std::string, std::map<cvedix_meta_trace_field, std::any>> trace_table;
    public:
        cvedix_meta(cvedix_meta_type meta_type, int channel_index);
        ~cvedix_meta();

        // the time when meta created
        std::chrono::system_clock::time_point create_time;

        cvedix_meta_type meta_type;
        ...
        virtual std::shared_ptr<cvedix_meta> clone() = 0;

        // attach a new trace record for specific node (initialize key-value for current node)
        //void attach_trace(std::string node_name);

        // update trace record
        //void update_trace(std::string node_name, cvedix_meta_trace_field trace_key, std::any trace_value);
    };
